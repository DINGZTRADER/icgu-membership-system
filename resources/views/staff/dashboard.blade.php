...
@section('content')
<div class="hero"><div><span class="eyebrow">Secretariat command centre</span><h2>Membership operations at a glance</h2><p>Monitor member standing, applications, renewals, arrears and collections from one permission-aware workspace.</p></div><span class="status info">{{ $roles->join(' · ') }}</span></div>
<div class="grid grid-4 metrics">
    ...
    <div class="card metric"><small>Open applications</small><strong>{{ number_format($stats['applications_open']) }}</strong><div class="delta">{{ $stats['applications_decision'] }} need a decision</div></div>
...
<div class="grid grid-3 metrics">
    ...
    <div class="card metric"><small>Open renewals</small><strong>{{ $stats['renewals_open'] }}</strong></div>
</div>

<div class="detail-grid section">
<section>
    <div class="section-head"><div><h3>Applications requiring attention</h3><p>Oldest open cases appear first.</p></div>@if(auth()->user()->hasPermission('applications.view'))<a class="btn btn-soft" href="{{ route('staff.applications.index') }}">View all</a>@endif</div>
    <div class="table-wrap"><table class="table"><thead><tr><th>Reference</th><th>Applicant</th><th>Plan</th><th>Status</th><th>Balance</th></tr></thead><tbody>
