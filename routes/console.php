...
Artisan::command('icgu:verify-sprint3', function (): int {
    ...
    try {
        ...
        if ($application->receipts()->count() !== 2) {
            throw new \RuntimeException('Expected one receipt per payment.');
        }

        $this->info('Sprint 3 invoicing, payments, receipts and admission verified successfully.');
        return 0;
    } finally {
        DB::rollBack();
    }
})->purpose('Exercise Sprint 3 invoice, partial/full payment, receipt and paid-member admission lifecycle.');
